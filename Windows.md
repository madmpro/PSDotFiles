...
## Usage
If you just want to run the script with default preset, simply right click on the *Windows.ps1* file, choose *Run with PowerShell*, and confirm execution policy change. Make sure your account is a member of *Administrators* group as the script attempts to run with elevated privileges.
...
To supply a customized preset, you can either pass the function names directly as parameters.
    ...
    powershell.exe -NoProfile -ExecutionPolicy Bypass -File Windows.ps1 EnableFirewall EnableDefender
...
Command using the preset file above:
    ...
    powershell.exe -NoProfile -ExecutionPolicy Bypass -File Windows.ps1 -preset mypreset.txt
