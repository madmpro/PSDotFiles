...
Set-Alias -Name 'whereis' -Value WhereIs
...
# Far
If (Test-Path "C:\Program Files\Far Manager\far.exe")
{
...
{
  if (!(Get-Command 'mc.exe' -ErrorAction SilentlyContinue | Test-Path)) {
...
{
  if (!(Get-Command 'nano.exe' -ErrorAction SilentlyContinue | Test-Path)) {
    Set-Alias -Name 'nano' -Value "C:\ProgramData\Chocolatey\bin\nano.exe"
  }
}
...
If (Test-Path "$PSScriptRoot\Private\prv_Aliases.ps1")
{
  . "$PSScriptRoot\Private\prv_Aliases.ps1"
}
