...
###############################################################################
#Get-ChildItem ( "$profile\Functions" + "*.ps1") | ForEach-Object {. (Join-Path "$profile\Functions" $_.Name)} | Out-Null
...
###############################################################################
function Hosts { Invoke-Expression "$(if(Get-Command 'npp') { 'npp' } elseif ($env:EDITOR -ne $null) {$env:EDITOR } else { 'notepad' }) $env:windir\system32\drivers\etc\hosts" }
function Edit-Profile { Invoke-Expression "$(if(Get-Command 'npp') { 'npp' } elseif ($env:EDITOR -ne $null)  {$env:EDITOR } else { 'notepad' }) $profile" }
...
Function RemoveRegistryDrives {
  Remove-PSDrive   -Name "HKCR" | Out-Null
  Remove-PSDrive   -Name "HKCC" | Out-Null
...
# String	          REG_SZ
# Unknown	          –
Function SetRegistryKey($RegPath, $RegKey, $RegType, $RegValue) {
  ...
  Set-ItemProperty -Path $RegPath -Name $RegKey -Type $RegType -Value $RegValue

}

Function DeleteRegistryKey($RegPath, $RegKey) {
